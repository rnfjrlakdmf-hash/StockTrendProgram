          ...
          <section aria-labelledby="blog-heading">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {blogPosts.length > 0 ? (
                ...
                // API 실패 시 노출되는 정적 Fallback 콘텐츠 (SEO 및 애드센스용)
                [
                  { id: 'fallback-1', title: '오늘의 글로벌 마켓: 나스닥 기술주 중심 반등, 반도체 섹터 강세 뚜렷', summary: '미국 나스닥 지수가 빅테크 실적 호조에 힘입어 상승 마감했습니다. 특히 엔비디아를 필두로 한 반도체 섹터로 자금이 유입되며 투심이 회복되는 모습입니다.' },
                  { id: 'fallback-2', title: '코스피 외국인 순매수 전환: 환율 안정화가 이끄는 대형주 장세', summary: '달러 강세가 주춤하며 외국인 자금이 코스피 대형주를 중심으로 유입되고 있습니다. 삼성전자, SK하이닉스 등 수출주 중심의 반등이 눈에 띕니다.' },
                  { id: 'fallback-3', title: '배당주 투자 시즌 돌입: 금융주와 통신주 매력도 점검', summary: '연말을 앞두고 배당주에 대한 관심이 높아지고 있습니다. 안정적인 현금 흐름을 바탕으로 고배당이 예상되는 주요 금융주들의 실적을 분석했습니다.' },
                  { id: 'fallback-4', title: 'FOMC 금리 동결 여파: 시장의 금리 인하 기대감 확산', summary: '미 연준이 기준금리를 동결함에 따라 시장에서는 내년 상반기 금리 인하에 대한 기대감이 커지고 있습니다. 채권 시장의 반응과 주식 시장의 영향 분석입니다.' }
                ].map((post) => (
                  <div key={post.id} className="group p-5 rounded-2xl bg-white/[0.03] border border-white/[0.07] opacity-80">
                    <p className="text-[11px] text-blue-400 font-bold mb-2 uppercase tracking-wide">마켓 브리핑</p>
                    <h3 className="text-white font-bold text-sm leading-snug mb-2 line-clamp-2">{post.title}</h3>
          ...
          <section aria-labelledby="theory-heading">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {theoryPosts.length > 0 ? (
                ...
                // API 실패 시 노출되는 정적 Fallback 콘텐츠 (SEO 및 애드센스용)
                [
                  { id: 'theory-fallback-1', title: '이동평균선의 이해: 5일선과 20일선이 만날 때 (골든크로스)', summary: '가장 기본적인 차트 분석 지표인 이동평균선의 개념과 단기/장기 이평선 교차를 통해 매수 타이밍을 잡는 기초 이론을 학습합니다.' },
                  { id: 'theory-fallback-2', title: 'RSI 지표 완전 정복: 과매수와 과매도 구간 판단하기', summary: 'RSI(상대강도지수)가 30 이하로 떨어졌을 때 분할 매수하는 전략과, 70 이상에서 리스크를 관리하는 실전 투자 기법을 알아봅니다.' },
                  { id: 'theory-fallback-3', title: '캔들 차트 기초 이해: 양봉과 음봉, 꼬리의 의미 파악하기', summary: '시가, 종가, 고가, 저가로 이루어진 캔들스틱 차트를 읽는 법과, 밑꼬리가 긴 양봉이 가지는 심리적 의미를 해석해 봅니다.' },
                  { id: 'theory-fallback-4', title: '거래량의 비밀: 주가 바닥에서 터지는 대량 거래량', summary: '주식 격언 중 "주가는 속여도 거래량은 속일 수 없다"는 말이 있습니다. 바닥권에서의 거래량 폭발이 의미하는 바를 분석합니다.' }
                ].map((post) => (
                  <div key={post.id} className="group p-5 rounded-2xl bg-white/[0.03] border border-white/[0.07] opacity-80">
                    <p className="text-[11px] text-indigo-400 font-bold mb-2 uppercase tracking-wide">투자 기초</p>
                    <h3 className="text-white font-bold text-sm leading-snug mb-2 line-clamp-2">{post.title}</h3>
                    <p className="text-gray-500 text-xs leading-relaxed line-clamp-2">{post.summary}</p>
                  </div>
                ))
