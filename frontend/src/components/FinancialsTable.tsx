...
export default function FinancialsTable({ data: rawData, currency }: FinancialsTableProps) {
    ...
    return (
        <div className="animate-in fade-in slide-in-from-bottom-2 duration-500 space-y-5 pt-1 w-full max-w-full">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3.5 pb-4 border-b border-white/10">
                <div className="flex items-center gap-3">
                    <div className="w-11 h-11 rounded-2xl bg-gradient-to-br from-amber-500/20 to-indigo-500/20 border border-amber-500/30 flex items-center justify-center shadow-inner shrink-0">
                ...
                {aiSummary && (
                    <div className="flex items-center gap-2 flex-wrap">
                        <div className="px-3 py-1.5 rounded-xl bg-zinc-950 border border-white/10 text-center shadow-inner">
                            ...
                            <div className="text-xs md:text-sm font-black text-white font-mono mt-0.5">{aiSummary.latestRev}</div>
                        </div>
                        <div className="px-3 py-1.5 rounded-xl bg-zinc-950 border border-white/10 text-center shadow-inner">
                            <div className="text-[9px] text-zinc-400 font-bold uppercase">최근 영업익</div>
                            <div className="text-xs md:text-sm font-black text-emerald-400 font-mono mt-0.5">{aiSummary.latestOp}</div>
                        </div>
                        <div className="px-3 py-1.5 rounded-xl bg-zinc-950 border border-white/10 text-center shadow-inner">
                            <div className="text-[9px] text-zinc-400 font-bold uppercase">최근 ROE</div>
                            <div className="text-xs md:text-sm font-black text-purple-300 font-mono mt-0.5">{aiSummary.latestRoe}</div>
                        </div>
                        <div className="px-3 py-1.5 rounded-xl bg-zinc-950 border border-white/10 text-center shadow-inner">
                            <div className="text-[9px] text-zinc-400 font-bold uppercase">부채비율</div>
                            <div className="text-xs md:text-sm font-black text-cyan-300 font-mono mt-0.5">{aiSummary.latestDebt}</div>
                        </div>
                    </div>
                )}
            </div>
            ...
            {aiSummary && (
                <div className="bg-gradient-to-br from-zinc-950 via-[#0c142e] to-zinc-950 border border-indigo-500/30 rounded-3xl p-4 md:p-5 shadow-2xl space-y-3.5 relative overflow-hidden">
            ...
            {activeMetricConfig && (
                <div className="p-4 md:p-4.5 rounded-3xl bg-zinc-950 border border-amber-500/35 shadow-2xl space-y-3 relative overflow-hidden animate-in fade-in duration-200">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 pb-2.5 border-b border-white/10">
                        <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-2xl bg-amber-500/15 border border-amber-500/30 text-amber-300 text-xl flex items-center justify-center shrink-0">
                    ...
                    <div className="p-3 rounded-2xl bg-amber-500/10 border border-amber-500/25 flex items-start gap-2.5 text-xs md:text-sm text-amber-200">
            ...
            {/* 4. 카테고리 필터 탭 바 (flex-wrap으로 우측 잘림 완전 방지) */}
                ...
                {CATEGORIES.map(cat => {
                    ...
                    return (
                        <button
                            ...
                            className={`px-3.5 py-2 rounded-2xl text-xs md:text-sm font-black transition-all flex items-center gap-1.5 shrink-0 cursor-pointer border ${
                                isActive 
            ...
            {activeCategory === 'div' && displayedGroups.length === 0 ? (
                ...
                <div className="bg-gradient-to-br from-zinc-950 via-[#120e28] to-zinc-950 border border-purple-500/30 rounded-3xl p-5 md:p-6 shadow-2xl space-y-4">
                    <div className="flex items-center justify-between flex-wrap gap-3 pb-3 border-b border-white/10">
                        ...
                        {dividendSummary?.hasDividend ? (
                            ...
                            <span className="px-3 py-1.5 rounded-xl bg-amber-500/20 text-amber-300 border border-amber-500/40 text-xs font-black flex items-center gap-1.5">
                ...
                /* 통합 마스터 테이블 (화면 우측 잘림 제로! 100% 한눈에 쏙 들어오는 퍼펙트 핏) */
                        ...
                        <table className="w-full text-left border-collapse min-w-[620px] sm:min-w-full">
                            <thead>
                                <tr className="bg-[#0b1020] border-b border-white/10">
                                    <th className="py-2.5 px-3 md:px-4 sticky left-0 bg-[#0b1020] z-30 w-32 md:w-36 border-r border-white/15 shadow-[3px_0_10px_rgba(0,0,0,0.7)]">
                                        <span className="text-[11px] md:text-xs font-black uppercase text-zinc-300 tracking-wider">주요 재무 지표</span>
                                    </th>
                                    <th colSpan={annualDates.length} className="py-2 px-1.5 text-[11px] md:text-xs font-black uppercase tracking-wider text-emerald-300 text-center border-b border-emerald-500/40 bg-emerald-950/30">
                                        📊 연간 실적 (Yearly)
                                    </th>
                                    {hasQuarterlyData && (
                                        <th colSpan={quarterlyDates.length} className="py-2 px-1.5 text-[11px] md:text-xs font-black uppercase tracking-wider text-blue-300 text-center border-b border-blue-500/40 bg-blue-950/30 border-l border-white/15">
                                            ⏰ 분기 실적 (Quarterly)
                                        </th>
                                    )}
                                </tr>
                                <tr className="border-b-2 border-indigo-500/40 bg-[#0e162e] text-xs">
                                    <th className="py-2.5 px-3 md:px-4 text-zinc-300 font-extrabold uppercase tracking-wider sticky left-0 bg-[#0e162e] z-30 backdrop-blur-md w-32 md:w-36 border-r border-white/15 shadow-[3px_0_10px_rgba(0,0,0,0.7)] whitespace-nowrap">
                                        지표명 (단위)
                                    </th>
                                    {dates.map((date: string, idx: number) => {
                                        ...
                                        return (
                                            <th 
                                                key={idx} 
                                                className={`py-2.5 px-1 sm:px-1.5 md:px-2 text-center whitespace-nowrap font-black font-mono text-[11px] sm:text-xs md:text-sm ${
                                                    isEstimate(date) 
                                ...
                                {displayedGroups.map((group) => (
                                    <React.Fragment key={group.title}>
                                        <tr className="bg-gradient-to-r from-[#111933] via-zinc-900 to-zinc-950 border-t-2 border-b border-white/15">
                                            <td className="py-2.5 px-3 md:px-4 font-black text-xs text-amber-300 uppercase tracking-wider sticky left-0 z-20 bg-[#111933] border-r border-white/15 shadow-[3px_0_10px_rgba(0,0,0,0.7)] whitespace-nowrap w-32 md:w-36">
                                                {group.title}
                                            </td>
                                            <td colSpan={dates.length} className="py-2.5 px-2.5 text-xs text-zinc-400 font-medium break-keep">
                                                • {group.desc}
                                            </td>
                                        </tr>
                                        ...
                                        {group.keys.map((key) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    <td className={`py-3 px-3 md:px-4 sticky left-0 z-20 backdrop-blur-md border-r border-white/15 shadow-[3px_0_10px_rgba(0,0,0,0.7)] whitespace-nowrap transition-colors w-32 md:w-36 ${
                                                ...
                                                {metric.values.map((val: any, idx: number) => {
                                                    ...
                                                    return (
                                                        <td 
                                                            key={idx} 
                                                            className={`py-3 px-1 sm:px-1.5 md:px-2 text-center whitespace-nowrap transition-colors ${
                                                        ...
                                                        >
                                                            {val === null ? (
                                                                ...
                                                                <div className="flex flex-col items-center gap-0.5">
                                                                    <span className={`tracking-tight text-xs sm:text-sm font-extrabold ${colorClass}`}>
            ...
            {/* 6. 하단 범례 & 데이터 공시 출처 */}
            <div className="p-3.5 bg-zinc-950/90 rounded-2xl border border-white/10 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-zinc-400 shadow-inner">
