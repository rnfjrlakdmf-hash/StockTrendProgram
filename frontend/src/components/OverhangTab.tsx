...
import { AlertTriangle, Building2, ExternalLink, Loader2, Info } from 'lucide-react';
...
export default function OverhangTab({ symbol, stockName }: OverhangTabProps) {
    ...
    useEffect(() => {
        ...
        fetch(`${API_BASE_URL}/api/stock/${symbol}/dart_overhang`)
            .then(res => res.json())
            .then(data => {
                if (!isMounted) return;
            ...
            .catch(err => {
                ...
                if (isMounted) setError('통신 중 오류가 발생했습니다.');
            })
            .finally(() => {
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center py-12 space-y-4">
                ...
                <p className="text-gray-400 font-bold">DART 공시 기록을 스캔 중입니다...</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            {/* Guide Message */}
            <div className="bg-gray-800/50 border border-gray-700 rounded-xl p-4 flex gap-3 text-sm text-gray-300">
