        ...
        <div className="w-full bg-black/40 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] overflow-hidden shadow-2xl animate-in fade-in slide-in-from-top-4 duration-700">
            ...
            <div className="p-7 md:p-10 space-y-12">
                ...
                {(currentBullets.length > 0) ? (
                    ...
                    <div className="p-8 border border-dashed border-white/10 rounded-3xl text-center bg-white/5 transition-all hover:bg-white/10">
                        <p className="text-gray-400 text-sm font-bold mb-4">새로운 네이버 스타일 브리핑 준비가 완료되었습니다!</p>
                        <button onClick={() => fetchBrief(true)} className="px-6 py-2.5 bg-blue-600 text-white font-black rounded-xl hover:scale-105 transition-transform shadow-xl shadow-blue-500/20">
                            지금 바로 업데이트 하기
                        </button>
                    </div>
                )}

                {/* 2. Segmented Sections & Watchlist */}
            ...
            {/* Footer */}
            <div className="px-10 py-8 bg-black/80 border-t border-white/5 space-y-6">
                <div className="flex items-start gap-4 p-5 bg-red-500/5 border border-red-500/10 rounded-[2rem]">
                    <AlertCircle className="w-6 h-6 text-red-500 shrink-0 mt-0.5" />
                    <div className="space-y-1">
                        <p className="text-[11px] font-black text-red-500 uppercase tracking-widest">Investment Disclaimer & Compliance Notice</p>
                        <p className="text-[10px] text-gray-500 leading-relaxed font-bold">
                            본 서비스는 인공지능 기반 데이터 분석 도구로, 투자 권유가 아닙니다. 모든 정보는 외부 데이터를 기반으로 자동 생성되며 정확성을 보장하지 않습니다. 최종 투자 책임은 본인에게 있습니다.
                        </p>
                    </div>
                </div>
                
                <div className="flex items-center justify-between pt-2">
                    <div className="flex items-center gap-6 text-[10px] font-black tracking-widest text-gray-500">
                        <span className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></div> ENGINE STATUS: OPTIMIZED</span>
                        <span className="flex items-center gap-2 bg-blue-500/20 text-blue-400 px-3 py-1 rounded-lg border border-blue-500/20">PREMIUM INTELLIGENCE</span>
                    </div>
                    <button onClick={() => fetchBrief(true)} className="px-6 py-2 bg-blue-600/10 hover:bg-blue-600/20 text-blue-500 font-black rounded-xl transition-all hover:scale-105 active:scale-95 uppercase tracking-widest text-[11px]">REFRESH DATA</button>
                </div>
            </div>
        </div>
    );
