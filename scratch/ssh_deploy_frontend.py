...
def deploy_frontend():
    ...
    try:
        ...
        ssh.exec_command("bash -l -c 'pm2 stop 0'")
        # build command: npm install && npm run build
        # 쉘 환경 변수 로드를 위해 bash -l -c 를 사용합니다. (node, npm path 관련)
        build_cmd = "bash -l -c 'cd ~/StockTrendProgram && git pull && cd frontend && rm -rf .next && npm install && NODE_OPTIONS=\"--max-old-space-size=512\" npm run build'"
