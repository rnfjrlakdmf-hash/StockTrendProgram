...
# 2) 강제 장마감 알림 호출 (미국)
try:
    print("[TEST] Triggering send_closing_notification('US') now...")
    send_closing_notification("US")
    print("[TEST] Completed without exception! (FCM multicast sent for US)")
except Exception as e:
    print("[TEST-Error] Failed to trigger US closing report:", e)
"""
        sftp = ssh.open_sftp()
        with sftp.open("/home/ubuntu/StockTrendProgram/backend/trigger_closing_test_us.py", "w") as f:
            f.write(py_code)
        sftp.close()
        
        # 2. 실행 및 결과 수집 (FastAPI 가상환경 내 python3 활용)
        cmd_run = "cd /home/ubuntu/StockTrendProgram/backend && ./venv/bin/python trigger_closing_test_us.py"
        stdin, stdout, stderr = ssh.exec_command(cmd_run)
        print("[STDOUT]\n", stdout.read().decode('utf-8', 'ignore'))
        print("[STDERR]\n", stderr.read().decode('utf-8', 'ignore'))
        
        # 3. 임시 파일 삭제
        ssh.exec_command("rm /home/ubuntu/StockTrendProgram/backend/trigger_closing_test_us.py")
        ssh.close()
    except Exception as e:
        print("Failed:", e)

if __name__ == "__main__":
    trigger_closing_test_us()
