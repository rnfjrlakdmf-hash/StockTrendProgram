...
async def generate_marketing_content(request: MarketingRequest):
    ...
    우리의 목적은 '이 글을 본 사람들이 무조건 우리 사이트(AI 주식 비서, stock-trend-program.co.kr)로 와서 데이터 분석을 해보게 만드는 것'입니다.

    [절대 엄수해야 할 법적 제약 조건 - 위반 시 처벌]
    1. 특정 종목의 "매수", "매도"를 직접적으로 지시하거나 권유하는 표현 절대 금지.
    2. 목표가 제시, 수익률 숫자 보장 절대 금지 (예: "300% 간다", "무조건 떡상", "전재산 풀매수" 등 리딩성 발언 금지).
    3. 오직 '데이터 분석', '시장 트렌드 확인', '인사이트 제공' 목적으로만 호기심을 유발할 것.
    4. 각 채널의 글 맨 마지막에는 반드시 다음 문장과 같은 의미의 면책 조항(Disclaimer)을 자연스럽게 1줄 추가할 것: "본 정보는 투자 참고용이며, 투자에 대한 모든 판단과 책임은 투자자 본인에게 있습니다."

    키워드/주제: {request.keyword}
    타겟 분위기: {request.tone}

    채널 1. 네이버 블로그 (Blog)
    - 정보성 글처럼 시작하지만 끝에는 'AI 주식 비서'를 써보니 객관적인 분석을 해줘서 좋다는 식으로 유도.
    - 적절한 이모지와 가독성 높은 줄바꿈 포함.

    채널 2. 주식 커뮤니티 (디시인사이드 주식갤러리, 네이버 종토방) (Community)
    - 어그로성 강한 제목 (예: "와 ㅅㅂ 000 물린 흑우들 있냐? ㅋㅋㅋ", "형이 팩폭 꽂아준다")
    - 짧고 굵게, 은어를 섞어서 극강의 바이럴 유도.
    - "AI 주식 비서에서 데이터 돌려보니까 팩폭 꽂히더라" 같은 내용 포함 (투자리딩이 아님을 명시).

    채널 3. 유튜브 쇼츠 / 틱톡 대본 (Shorts)
    - 첫 3초 시선을 끄는 강력한 훅(Hook).
    - 60초 이내에 읽을 수 있는 짧은 대본.
    - 마지막에 무조건 우리 사이트 주소("stock-trend-program.co.kr")를 직접적으로 언급하며 방문을 강하게 유도할 것.

    채널 4. 인스타그램 피드/쓰레즈 (Instagram)
    - 쓰레즈(Threads) 텍스트 길이 제한에 맞추어 **본문 내용을 3~4문장 이내(해시태그 제외 공백 포함 200자 내외)**로 아주 짧고 강력하게 요약할 것. 절대 길게 쓰지 마세요.
    - 시각적인 이모지를 다수 사용하고, 모바일 가독성을 극대화.
    - 검색 유입을 위한 주식 관련 해시태그 5~7개 정도 포함.
    - 본문 내에 반드시 우리 사이트 주소("stock-trend-program.co.kr" 또는 "https://stock-trend-program.co.kr")를 명시하고, 접속해서 확인해보라는 홍보 문구를 눈에 띄게 추가할 것.

    반드시 아래 JSON 형식으로만 응답해주세요. (다른 텍스트 추가 금지)
    {{
        "blog": {{
            "title": "블로그용 제목",
            "content": "블로그 본문 (면책조항 포함)"
        }},
        "community": {{
            "title": "어그로성 제목",
            "content": "커뮤니티 본문 (면책조항 포함)"
        }},
        "shorts": {{
            "title": "숏츠용 훅 제목",
            "script": "쇼츠 대본 (영상 자막용 면책조항 포함)"
        }},
        "instagram": {{
            "title": "인스타그램 제목/첫줄",
            "content": "인스타그램 본문, 해시태그 및 면책조항"
        }}
    }}
    """
