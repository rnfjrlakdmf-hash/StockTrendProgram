...
async def generate_user_morning_briefing(user_id: str):
    ...
    [작성 가이드라인 - 필독 및 엄수]
    - **중복 절대 금지**: 상단(`market_summary`)과 하단(`market_focus`)의 정보가 1%도 겹치지 않게 하세요.
        - `market_summary`: 오직 **지수 등락 결과와 시장의 핵심 테마(무엇이 올랐나)**만 **최대 2문장**으로 작성하세요. (지표, 지수 일정 절대 금지)
        - `market_focus`: 오직 **관찰해야 할 지표(PPI 등)와 공시 일정**만 불렛 포인트(•)로 나열하세요. 다른 미사여구는 모두 생략하세요.
    - **가독성 극대화**: 모든 텍스트는 **요약형(단축형)**으로 작성하세요.
    - **전문가 버전 vs 초보자 버전**: 모든 주요 항목에 대해 전문가용(`insight` 등)과 초보자용(`simple_` 등)을 함께 작성하세요.
    - **쉬운 설명 기준**: 어려운 용어는 비유로 풀어서 중학생도 이해할 수 있게 하세요.
    - **실시간 데이터 기반**: 제공된 데이터에만 기초하여 분석하세요.
    - **추측 금지**: 불분명한 경우 '데이터 수집 중'으로 표시하세요.
    - 말투: 전문가는 격식 있게, 초보자는 친근하게 작성하세요.
    - **투자 자문 금지**: 가격 제시, 매수/매도 추천은 절대 불가합니다.
