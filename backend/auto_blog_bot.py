...
def generate_market_post(market_type):
    ...
    if market_type == "kor":
        ...
        작성 가이드:
        1. 맨 첫 줄에 반드시 사람들이 네이버나 구글에 검색할 만한 롱테일 키워드를 포함한 SEO 최적화된 제목(예: "엔비디아 주가 급등 원인 및 {today_str} 미국 증시 시황 AI 요약 (테슬라 전망 포함)")을 <title-seo>여기에 작성</title-seo> 형태로 출력하세요. 제목과 본문에는 검색 노출이 잘 되는 인기 종목 이름(예: 엔비디아, 테슬라, 애플 등)을 자연스럽게 1~2개 포함하세요.
        2. 본문 첫 제목은 `<h2 class="text-2xl font-bold text-white pb-2 border-b border-gray-700 mb-6">🚀 [SEO제목 그대로 삽입]</h2>` 로 작성하세요.
