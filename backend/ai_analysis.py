...
def generate_stock_briefing(symbol: str) -> Dict[str, Any]:
    ...
    투자자가 지루하지 않게 **아주 짧고 임팩트 있는 3줄 브리핑**을 작성해주세요 (최대 150자 이내).

    [작성 규칙]
    1. 긴 서술어("~을 하였습니다", "~을 보여주고 있습니다")를 피하고, 명사형이나 짧은 문장("~함", "~돌파", "~증가세")으로 끝내세요.
    2. 의미없는 단순 현재가 나열을 피하고, 왜 올랐는지(혹은 떨어졌는지) 핵심 이슈나 수급 변화 1가지만 강력하게 짚어주세요.
    3. 각 줄의 시작엔 직관적인 이모지(🔥, 📈, 📉, 🏢, 💡 등)를 사용하세요.
    4. [매우 중요] 수급을 설명할 때는 외국인/기관의 **숫자(주(株))와 순매수/순매도 여부**를 반드시 명확하게 표기하세요. (예: "외국인 15,300주 순매도", "기관 30만주 폭풍 순매수")
    5. 매수/매도 권유 금지. 철저히 객관적 사실만 짧게 나열.
