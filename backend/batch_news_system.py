...
GENERIC_EXCLUDE_PATTERNS = [
    ...
    r'주가.*\d+원.*(가격\s*변동\s*없이|변동\s*없이|보합\s*마감|변동\s*없어)', # 예: 삼성중공업 주가, 9월 7일 21,400원 가격 변동 없이 마감
    r'(가격\s*변동\s*없이|변동\s*없이|보합\s*마감)\s*(마감|기록|종가)',
    r'(외국인|기관|개인|투신).*(순매수|순매도|팔고|사고)', # 예: 외국인/기관 순매수 상위 종목
    r'\[빅데이터 리포트\]',            # 기계 생성 리포트
    r'(투자|매매)\s*동향',             # 매매 동향 나열
    r'거래량\s*(급증|폭발)',           # 단순 거래량 알림
    # ── [추가] 대형주 스팸 방지 (코스피/코스닥 시황 브리핑) ──
    r'(코스피|코스닥|증시|뉴욕증시|KOSPI).*(상승|하락|마감|출발|돌파|턱걸이|붕괴|보합)',
    r'(코스피|코스닥)\s*\d{3,4}선',    # 코스피 2700선, 코스닥 800선
    r'시총 상위',                     # 시총 상위주 나열
    r'특징주\s*\[',                   # [특징주] 코너 전체적인 나열 방지
]
...
class BatchNewsSystem:
    ...
    async def _send_fcm(
        ...
        try:
            ...
            # 본문: 제목 + 언론사 출처 (특수문자 제거하고 띄어쓰기로만 구분)
            push_body = clean_title.replace("[", "").replace("]", "").replace("|", "")
            if publisher:
