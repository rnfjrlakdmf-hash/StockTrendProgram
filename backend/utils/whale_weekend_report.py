...
def fetch_whale_top3():
    ...
    try:
        ...
        for row in foreign_rows:
            ...
            if name_cell:
                foreign_top3.append(name_cell.text.strip())
                if len(foreign_top3) == 3:
                    break
        ...
        for row in inst_rows:
            name_cell = row.find('a', class_='company')
            if name_cell:
                inst_top3.append(name_cell.text.strip())
                if len(inst_top3) == 3:
                    break
...
def _generate_whale_report_sync():
    ...
    foreign_top3, inst_top3 = fetch_whale_top3()
    if not foreign_top3:
        foreign_top3 = ["데이터 수집 실패"]
    if not inst_top3:
        inst_top3 = ["데이터 수집 실패"]
        
    print(f"[WhaleReport] Foreign: {foreign_top3}, Inst: {inst_top3}")
