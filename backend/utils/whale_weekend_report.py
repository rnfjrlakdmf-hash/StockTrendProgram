...
3. monday_strategy: 전문 금융 용어(수급 요약, 스마트머니, 눌림목, 포트폴리오 압축 등)의 전문성을 유지하면서, 초보자도 쉽게 이해할 수 있는 친절한 괄호/보충 설명을 덧붙여 3개 항목으로 작성하세요 (줄바꿈 포함):
   • 📌 수급 핵심 요약: 외국인과 기관(양대 큰손)의 매수 자금이 반도체 대장주(삼성전자·SK하이닉스)와 K-수출 실적 우량주로 집중 압축되었습니다.
   • 🎯 공략 포인트 (눌림목 매매): 스마트머니(큰손 자금)가 유입된 실적주는 급등 시 추격 매수하기보다, 주가가 잠시 숨고르기(조정·눌림목)를 거칠 때 분할 매수로 접근하는 것이 유리합니다.
   • 🛡️ 리스크 관리 (종목 압축): 단기 급등 테마주를 무리하게 쫓아가기보다, 큰손들의 연속적인 순매수가 확인된 튼튼한 대형 우량주 위주로 보유 종목을 좁히는(압축하는) 전략이 안전합니다.
...
{{
  ...
  "monday_strategy": "• 📌 수급 핵심 요약: 외국인과 기관(양대 큰손)의 매수 자금이 반도체 대장주(삼성전자·SK하이닉스)와 K-수출 실적 우량주로 집중 압축되었습니다.\n• 🎯 공략 포인트 (눌림목 매매): 스마트머니(큰손 자금)가 유입된 실적주는 급등 시 추격 매수하기보다, 주가가 잠시 숨고르기(조정·눌림목)를 거칠 때 분할 매수로 접근하는 것이 유리합니다.\n• 🛡️ 리스크 관리 (종목 압축): 단기 급등 테마주를 무리하게 쫓아가기보다, 큰손들의 연속적인 순매수가 확인된 튼튼한 대형 우량주 위주로 보유 종목을 좁히는(압축하는) 전략이 안전합니다.",
